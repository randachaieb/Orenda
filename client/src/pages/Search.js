...
const Search = (props) => {
    ...
    const handleFollow = async (profileId) => {
        try {
            const res = await axios.post(`http://localhost:5000/api/v1/user/follow/${profileId}`,
                null, {
                    headers: {
                        'x-auth-token': localStorage.getItem('token')
                    }
            ...
            if (res.status === 200) {
                ...
                const profileIndex = searchedResults.findIndex((profile) => profile._id === profileId)
                                ...
                                <div className="text-center card-box">
                                    <div className="member-card pb-2">
                                        ...
                                        <div>
                                            ...
                                            <button type="button" onClick={() => handleFollow(searchedResult._id)} className="btn btn-default mt-3 btn-rounded waves-effect w-md waves-light"><PersonAddIcon /> Follow</button>

                                            <button type="button" onClick={() => { history.push(`/user/${searchedResult._id}`) }} className="btn btn-default mt-3 btn-rounded waves-effect w-md waves-light"><VisibilityIcon /> View profile</button>
